...
namespace IccProfileNet.Tags
{
    ...
    internal sealed class IccLutABType : IccTagTypeBase, IIccClutType
    {
        ...
        #region Processor
        private double[] ProcessAtoB(double[] values, IccProfileHeader header)
        {
            double[] result = values.ToArray();

            if (HasACurve)
            {
                result = ProcessCurves(result, ACurves!);
            }
            ...
            if (HasMCurves)
            {
                result = ProcessCurves(result, MCurves!);
            }

            if (HasMatrix)
            {
                // process
                result = ApplyMatrix(result, header);
            }

            // BCurves
            return ProcessCurves(result, BCurves!);
        }

        private double[] ProcessBtoA(double[] values, IccProfileHeader header)
        {
            // BCurves // TODO check dims
            double[] result = ProcessCurves(values, BCurves);
            ...
            if (HasMCurves)
            {
                result = ProcessCurves(values, MCurves!);
            }

            if (HasACurve)
            {
                result = ProcessCurves(values, ACurves!);
            }
